app.controller("HomeCtrl", ["$scope", "$http",  "$routeParams", "$location", "$route", function($scope, $http, $routeParams, $location, $route) {
  $http.get('https://localhost:8080/api/publications/')
...
app.controller("NewCtrl", ["$scope", "$routeParams", "$http", "$route", "$location", function($scope, $routeParams, $http, $route, $location) {
  ...
  $http.get('https://localhost:8080/api/publications/' + $routeParams.id)
  .then(function(response) {
    console.log(response)
  ...
  $http.put('https://localhost:8080/api/publications/' + publication._id, publication)
    .then(function(response) {
      console.log("added");
  }, function(response) {
    console.log("Invalid URL");
...
$http.get('https://localhost:8080/api/publications/' + $routeParams.id)
  ...
  $scope.editPublication = function(publication) {
    ...
    $http.put('https://localhost:8080/api/publications/' + $routeParams.id, publication)
      .then(function(response) {
       console.log("update");
       $location.path('/');
     }, function(response) {
  ...
  $scope.deletePublication = function(publication) {
    ...
    $http.delete('https://localhost:8080/api/publications/' + publication._id, publication)
    .then(function(response) {
      $route.reload()
    }, function(response) {
      console.log("Invalid URL")
